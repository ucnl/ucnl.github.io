...
Только для системы RWLT, работающей в режиме определения местоположения водолазов. При помощи данного сообщение передаются определяемые навигационные данные нумерованной цели (например, водолаза, оснащенного устройством беспроводной голосовой связи [RedPhone-DX](/documentation/RU/RedPhone/RedPhone_DX_Specification_ru)). Идентификатор (адрес) объекта передается самим объектом и может принимать значения от 0 до 999.
...
| 1 | bID | Номер (адрес) буя, 0 .. 3 (соответствует буям 1 .. 4) |
