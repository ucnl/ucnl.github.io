...
- режим совместимости с трекинговой системой [RWLT]
- Адрес (идентификатор водолаза) для трекинговой системы
- Идентификатор канала системы [RWLT]() (Введен для будущих версий, должен иметь значение 0)
...
Беспроводное соединение устанавливается монопольно, это означает что в один момент времени работа по настройке проводится только с одним прибором RedPhone-DX.
...
Для того, чтобы записать настройки в станцию, нажмите кнопку **APPLY SETTINGS**. По результатам отобразится всплывающее окошко, подтверждающее успешную запись новых настроек или сообщающее об ошибочной ситуации (например, если донгл и/или станция были выключены).
