...
* [1. A³T и A³R](#1-a3t-и-a3r)
  ...
  * [1.5. (Планируется) Проект 3 - Гидроакустический модем]()
    * [1.5.1. (Планируется) Требуемый набор оборудования]()
  * [1.6. (Планируется) Проект 4 - Определение положения ответчика методом виртуальной длинной базы]()
    * [1.6.1. (Планируется) Требуемый набор оборудования]()
  * [1.7. (Планируется) Проект 5 - Длиинобазисная навигационная система]()
    * [1.7.1. (Планируется)Требуемый набор оборудования]()
    * [1.7.2. (Планируется)Навигационный буй-приемник]()
  * [1.8. (Планируется) Проект 6 - Фазированная антенная решетка]()
...
Подытожим: в результате экспериментов "на столе" мы:
 ...
 - выявили наличие статической ошибки и даже немного проировали распределение ее величины;
...
Распределение измеренного времени распространения, выполненное по 150 ищзмерениям имеет такой вид:
...
При проведении экспериментов на водоемах, полезно вести запись сигналов. Например, в обсуждаемых испытаниях гидрофон, на который велась запись, располагался в непосредственной близости от антенны запрашивающего устройства и ниже преставлен снимок экрана, на котором представлен фрагмен с одинм циклом "запрос-ответ".
...
Согласно этому фрагменту, между фронтами запросного и ответного сигнала временной промежуток в 711 мсек.

Если учесть фиксированную задержку ответа в 500 мсек, а остаток поделить на два - путь "туда-обратно", а еще учесть определенную в [п. 1.4.5.1.](#1451-на-столе) статическую ошибку в 1.096 мсек и скорость звука 1500 м/с, то согласно записи, наклонная дальность составит `((0.711-0.500) / 2 - 0.001096) * 1500 = 156.6 м`, что отлично согласуется и с измерением при помощи лазерного дальномера и с тем, что отображается на экране макета.
...
- акустические измерения согласуются с эталонными, выполненными при помощи лазерного дальномера
