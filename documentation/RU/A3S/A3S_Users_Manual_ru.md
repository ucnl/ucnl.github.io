...
* [1. A³T и A³R](#1-a3t-и-a3r)
  ...
  * [1.6. Проект 4 - Антенная решетка: измерение угла прихода сигнала](#16-проект-4---антенная-решетка-измерение-угла-прихода-сигнала)
    * [1.6.1. Требуемый набор оборудования](#161-требуемый-набор-оборудования)
    * [1.6.2. Пингер](#162-пингер)
    * [1.6.3. Приемник](#163-приемник)
      * [1.6.3.1. Скетч для обработки антенной решетки](#1631-скетч-для-обработки-антенной-решетки)
    * [1.6.4. Эксперимент по определению амплитуды разброса времени прихода](#164-эксперимент-по-определению-амплитуды-разброса-времени-прихода)
    * [1.6.5. Антенная решетка](#165-антенная-решетка)
    * [1.6.6. Эксперимент в бассейне](#166-эксперимент-в-бассейне)
    * [1.6.7. Обработка результатов эксперимента](#167-обработка-результатов-эксперимента)
...
Если все собрано правильно, то в окне утилиты 'Serial Monitor' мы сможем увидеть приходящие раз в 4 секунды строчки, содержащие по 4 числа через запятую. Скетч _нормализует_ времена прихода - вычитает наименьшее из каждой группы. 
...
В таких случаях логично увеличить число приемников и пусть голос каждого будет учтен.
...
### 1.7. (В Работе) Проект 5 - Определение положения ответчика методом виртуальной длинной базы

В ходе данной работы мы будем определять географическое местоположение _неподвижного_ подводного объекта - его широту и долготу. 
Для этого мы изготовим два приемопередатчика, которые уже знакомы нам по проекту [1.4. Проект 2 - Собираем приемопередатчик и измеряем наклонную дальность](#14-%D0%BF%D1%80%D0%BE%D0%B5%D0%BA%D1%82-2---%D1%81%D0%BE%D0%B1%D0%B8%D1%80%D0%B0%D0%B5%D0%BC-%D0%BF%D1%80%D0%B8%D0%B5%D0%BC%D0%BE%D0%BF%D0%B5%D1%80%D0%B5%D0%B4%D0%B0%D1%82%D1%87%D0%B8%D0%BA-%D0%B8-%D0%B8%D0%B7%D0%BC%D0%B5%D1%80%D1%8F%D0%B5%D0%BC-%D0%BD%D0%B0%D0%BA%D0%BB%D0%BE%D0%BD%D0%BD%D1%83%D1%8E-%D0%B4%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D1%8C). Один из них будет "подводным" объектом, положение которого мы будем определять, а второй - будет частью измерительного устройства. 

Как мы будем определять географическое положение - широту и долготу объекта?

Вспомним, что измеренная дальность определяет объект на сфере. Если вертикальная координата известна, то на круге, радиус которого определяется измеренной дальностью.
А теперь, измеряя дальность из нескольких точек мы "рисуем" несколько таких окружностей. И положение искомого (искомый - тот, который ищется) объекта будет в месте пересечения этих окружностей. 

Теперь представим, что мы измерили дальность до объекта из двух разных точек (например, с двух кораблей или с одного корабля в разное время). Каждое измерение задаёт свою окружность:

Если две окружности касаются в одной точке — это идеально: объект точно там.
Если они пересекаются в двух точках — мы не знаем, какая из них правильная. Нужно ещё одно измерение (ещё одна окружность), чтобы выбрать правильное место.

На практике измерения всегда неточные, поэтому окружности могут вообще не пересекаться - например, если мы использовали заниженное значение скорости звука. Тогда мы ищем точку, которая «почти» одинаково удалена от всех окружностей — то есть, наилучшее приближение.

Такие задачи называют оптимизационными (или минимизационными): мы ищем положение объекта, которое наилучшим образом _согласуется_ со всеми измерениями.

Каждая пара измерений из двух разных точек — это линия. Измерительная база, или просто - _базовая линия_. Но в нашем случае она не физическая, а виртуальная: измерения делаются не одновременно, а по очереди, с одного и того же корабля, который перемещается.

Такой способ работает только если объект неподвижен. Если он движется, то за время перемещения корабля к следующей точке объект успеет сместиться, и мы будем измерять расстояния до разных его положений. Тогда все окружности не сойдутся, и определить положение правильно не получится.

Очевидно, что нам потребуются два приемопередатчика: нужно _запросить_ объект, положение которого мы хотим определить, он должен этот запрос _принять_, затем он должен _излучить_ ответный сигнал, который нужно _принять_ уже на запрашивающем или измерительном устройстве.

Чтобы как-то привязаться с географии, нам необходим какой-нибудь GNSS-приемник. И далее, результаты работы надо куда-то выводить. В самом минимальном случае просто через последовательный порт на ПК, так же можно использовать какой-нибудь экран или дополнительно оборудовать измерительное устройство радиомодемом. 

Итак, вот список того, что нам потребуется:

#### 1.7.1. Требуемый набор оборудования
...
| 3    | Антенна приемопередающая [RT-1.332820-1](/documentation/RU/Transducers/RT_1_332820_1_Specification_ru) | 2 |  |
| 4    | Любая плата с МК, например, Arduino Nano | 2 |  |
| 5    | GNSS-приемник, почти любой, который можно подключить к используемой плате с МК | 1 | |
| 6    | Радиомодуль 433 МГц, например, HC-12 или совместимый | 2 | Опция |
| 7    | LCD-экран, например MT-204S | 1 | Опция |
| 7    | Провода Dupont Male-Female или Female-Female, 15+ см | 8 | |

Если вы планируете изготовить самый минимально функциональный вариант оборудования, то экран и радиомодуль не потребуются. В этом случае все результаты работы системы могут выподится просто через последовательный порт в ПК.
