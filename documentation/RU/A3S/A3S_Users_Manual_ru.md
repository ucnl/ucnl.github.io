...
| 3    | Антенна приемопередающая [RT-1.332820-1](/documentation/RU/Transducers/RT_1_332820_1_Specification_ru) | 2 |  |
| 4    | Любая плата с МК, например, Arduino Nano | 2 |  |
| 6    | Провода Dupont Male-Female или Female-Female, 15+ см | 6 | |

Дополнительно, как и для предыдущих опытов, потребуются источники питания для "кубиков", а так же два ПК или один ПК с возможностью подключения двух плат Arduino. 
Еще потребуется какое-нибуль терминальное ПО, чтобы была возможностью посылать данные в последовательный порт и отображать данные, пришедшие из последовательного порта.
Например, вполне сгодится утилита 'Serial monitor' из Arduino IDE, но если предполагается использовать один и тот же ПК для подключения обоих модемов, то это будет затруднительно. 
...
В качестве источника питания удобно использовать сборку 3S из Li-Ion или LiFePO4 аккумуляторов.
