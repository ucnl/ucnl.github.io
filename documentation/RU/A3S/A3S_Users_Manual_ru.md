...
В ходе данной работы мы будем определять географическое местоположение _неподвижного_ подводного объекта. 
...
Географическое положение - широта и долгота, будут определяться следующим образом: если объект, положение которого нужно найти неподвижен, например лежит на дне, то измерив до него дальность из нескольких точек можно решить эту задачу. 

Вспомним, что измеренная дальность определяет объект на сфере. Если вертикальная координата известная, то на круге, радиус которого определяется измеренной дальностью.
...
Какие могут быть варианты? Если у нас всего две окружности, то есть, мы измерили дальности до объекта из двух точек, то они могут:
- касаться в одной точеке - это идеальный случай, который позволит сразу решить задачу;
- пересекаться в двух точках - тогда возникает неоднозначность и для выбора правильного решения нужны дополнительные измерения - окружности;
- в реальном мире измерения выполняются с некоторой ошибкой и в нашем случае это может означать, что окружности могут не касаться вообще - например, если мы используем сильно заниженное значение скорости звука.

Это все означает, что эта задача, как и большинство других в навигации (и жизни), является оптимизационной, или, минимизационной. 



Этот проект будет заключаться в создании VLBL-системы. VLBL - значит Virtual Long Base-Line, т.е. Длиннобазисная навигационная система с длинной базой.
