...
* [1. A³T и A³R](#1-a3t-и-a3r)
  ...
  * [1.4. Проект 2 - Собираем приемопередатчик и измеряем наклонную дальность](#14-проект-2---собираем-приемопередатчик-и-измеряем-наклонную-дальность)
    ...
    * [1.4.5. Эксперименты]()
      * [1.4.5.1. На столе]()
      * [1.4.5.2. Пластиковый бак]()
      * [1.4.5.3. Плавательный бассейн]()
      * [1.4.5.4. Водоем]() 
   - [_1.5. (Планируется) Проект 3 - Определение положения ответчика методом виртуальной длинной базы_]()
     - [_1.5.1. (Планируется)Требуемый набор оборудования_]()
   - [_1.6. (Планируется) Проект 4 - Длиинобазисная навигационная система_]()
     - [_1.6.1. (Планируется)Требуемый набор оборудования_]()
     - [_1.6.2. (Планируется)Навигационный буй-приемник_]()
   - [_1.7. (Планируется) Проект 5 - Фаированная антенная решетка_]()
...
Интересно выглядит распределение величины статической ошибки - это очень похоже на т.н. Гауссову сместь (Gaussian Mixture), т.е. смесь нормальных (Гауссовых) распределений. Бимодальность может говорить о наличии неких двух сценариев, по которым происходит процесс.

| ![image](https://github.com/user-attachments/assets/e5c24575-9e79-4ffc-ba5a-975b0160dac2) |
| :---: |
| _Распределение величины статической ошибки_ |
...
Мы же для упрощения учтем измеренную статическую ошибку путем вычитания ее среднего значения. Введем `#define MAGIC_STATIC_S    (0.001096253)`, и будем ее вычитать при расчете наклонной дальности:
...
Вот, к примеру, запись работы лабораторной установки в пластиковом баке емкостью 350 л. Антенны в нем расположены на расстоянии неболее 20 см друг от друга:

| ![image](https://github.com/user-attachments/assets/282c9dcc-787d-4db8-a0b7-a7e3957ab381) |
| :---: |
| _Фрагмент записи работы запрашивающего устройства и маяка-ответчика_ |

Мы можем видеть, что между фронтами запросного и ответного сигнала 503 миллисекунды.


##### 1.4.5.3. (Планируется) Плавательный бассейн

##### 1.4.5.4. (Планируется) Водоем


### 1.5. (Планируется) Проект 3 - Определение положения ответчика методом виртуальной длинной базы

#### 1.5.1. (Планируется) Требуемый набор оборудования

### 1.6. (Планируется) Проект 4 - Длиинобазисная навигационная система

#### 1.6.1. (Планируется)Требуемый набор оборудования

#### 1.6.2. (Планируется)Навигационный буй-приемник

### 1.7. (Планируется) Проект 5 - Фаированная антенная решетка

### 1.7.1. (Планируется)Требуемый набор оборудования

## 2. (Планируется) A<sup>3</sup>TC и A<sup>3</sup>RC

## 3. (Планируется)A<sup>3</sup>AM
