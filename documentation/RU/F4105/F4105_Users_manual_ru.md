...
> ВНИМАНИЕ! Если модуль программиования и управления включается и при этом, тумблер (11) **Запрос** находится в положении **Вкл** (что не рекомендуется), то непосредственно перед переходом прибора в режим **Запрос** будет выдержана пауза примерно 30 секунд, в течение которой индикатор (5) **Запрос** будет моргать.
