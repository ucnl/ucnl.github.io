[Главная](/README_RU) ❯ [Гидроакустические модемы](/underwater_acoustic_modems_ru) ❯ **Описание протокола сопряжения: uWave**
...
Формат сообщения: **`$PUWV3,x,x,x.x,x.x,x.x,x.x*hh<CR><LF>`**                            
...
Формат сообщения: **`$PUWV5,x,x.x,x.x*hh<CR><LF>`**                
...
Формат сообщения: **`$PUWV!,c--c,с--с,x,c--c,x,x.x,x,x,x,x.x,x,x*hh<CR><LF>`**                            
